...
- `notices` table과 명명된 status/title/slug/published/window/actor FK constraint
- DB default draft·pinned false와 duplicate slug·게시 필수값·모든 상태의 기간 무결성 최종 차단
...
- invalid slug, duplicate, malformed timestamp, 길이 경계, unknown/id/audit field의 400/409 계약
- 게시 필수값과 draft/published/archived 기간 오류의 422 코드 분리
- partial `PUT`, 게시·기간 validation 실패 뒤 row·audit 전체 불변
- 미래 publishedAt 허용과 offset timestamp의 UTC 저장·응답
