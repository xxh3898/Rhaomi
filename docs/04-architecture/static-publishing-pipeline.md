...
- 공개 홈은 Shop·Service·Gallery·Notice snapshot을 정적 HTML에 포함하고 공지는 `/notices/<slug>/`로 export한다. `markdown-it 15.0.1`은 raw HTML을 비활성화하며 link protocol allowlist와 remote Markdown image의 alt-only 처리를 사용한다.
...
- validator는 required route, HTML parse, internal link, canonical, sitemap·robots, admin noindex, generated media byte size·SHA filename, unexpected symlink/special file와 credential/internal/private marker 부재를 검사한다. candidate를 loopback read-only server로 열어 홈·대표 공지·media·404를 다시 확인한 뒤에만 install/switch 단계로 이동한다.
- candidate는 `<releaseRoot>/.candidate-*`에서 완성하고 site tree digest와 canonical string revision/generation, generatedAt, code SHA·image tag/digest, Flyway version, SBOM reference를 strict private `release-manifest.json`에 기록한 뒤 same-filesystem rename으로 immutable release를 설치한다.
