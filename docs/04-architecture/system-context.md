...
7. HomeOps는 fixed privacy-safe status와 deployment/backup event만 읽고 관리자 콘텐츠 권한을 갖지 않는다. D-IMP-5b automatic decision source는 구현됐지만 production은 비활성이다. 후속 mapping은 public HTTPS/keyword 3회 실패의 `rhaomi-web` 하나뿐이고 backend는 unmapped/default-none이다.
