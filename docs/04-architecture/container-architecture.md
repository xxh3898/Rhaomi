...
local private media volume·upload API, same-origin development gateway, dedicated publisher control loop와 Build API→transformer→Next→immutable release/atomic switch data plane을 구현했다. network-disabled Node suites, explicit `publisher-validation`, `publication-acceptance` Java 25/Node 24 harness가 Sharp·filesystem symlink·DB completion에 더해 actual Admin HTTP·scheduled boundary·backend 중단 후 static serving을 amd64/arm64에서 검증한다. 위 production topology·lossless wire·verified-empty first activation·initial-admin·initial-content 계약은 [ADR-010](../09-decisions/ADR-010-production-topology-and-code-release.md)~[ADR-018](../09-decisions/ADR-018-production-initial-content-authority.md)에서 승인했다. D-IMP-1 decoder-only application image, D-IMP-2 `compose.production.yaml`·project Nginx, D-IMP-3 release/deploy, D-IMP-4 fixed backup/manifest/eligibility와 task-scoped isolated restore에 이어 no-port first-activation recovery Compose, exact-image initial-admin과 tracked-bundle initial-content source를 구현했다. actual Secret·Mac ownership/path·production volume·backup repository/schedule·private GHCR·Environment·Tailscale·ingress/deploy·backup/restore·HomeOps provisioning, 실제 관리자 생성·콘텐츠 import/publication은 여전히 미완료다.
...
- backend와 publisher의 service-specific argv/profile·container target은 D-IMP-2 Compose가 고정한다. D-IMP-3은 같은 image를 사용하는 opt-in `migration`·`schema-validate` task와 digest apply 경계를 연결하고, ADR-017/018은 같은 image의 opt-in `initial-admin`/`initial-content` task를 추가한다. D-IMP-4는 image에 built-in Node backup tool만 포함해 network-disabled opt-in service로 실행한다. D-IMP-5a status/event/recovery entrypoint는 container나 image에 Docker socket을 추가하지 않는 host-side fixed inventory다. UID/GID, Mac bind ownership, production Secret·backup repository와 actual registry identity는 host/external provisioning에서 확정한다. Dockerfile은 이를 임의 결정하는 `ENTRYPOINT`나 production 값을 bake하지 않는다.
...
`compose.production.yaml`은 default runtime에서 external exact image만 사용하는 `rhaomi-web`, `backend`, `publisher`, `postgres` 네 service를 canonical inventory로 둔다. `production-task` profile의 `migration`·`schema-validate`·`initial-admin`·`initial-content`와 `production-backup` profile의 `backup-tool`·`backup-verifier`는 같은 application image의 restart 없는 opt-in process다. Initial-admin은 read-only root, capability 0, no-new-privileges, port·host mount 0, `data-internal` only로 제한하고 raw account credential을 environment나 Compose에 받지 않는다. Initial-content는 같은 process hardening과 `data-internal`만 사용하고 owner bundle RO·canonical media RW 외 mount와 build/WebAuthn/admin credential을 갖지 않는다. HomeOps current generic control label은 read-only public bind와 tmpfs만 가진 `rhaomi-web` 하나에만 두며 다른 service에는 없다. backup tool은 network·port·Docker socket·DB/build credential 없이 provisioned repository와 deploy state만 RW, canonical media만 RO로 mount한다. deploy 전 full-read 전용 backup verifier는 root/repository/deploy-state RO, network none이며 media·credential·Docker socket·port가 없다. publisher image source는 read-only이며 Next/Turbopack build에 필요한 `.rhaomi-publication-work` target만 Mac publisher state의 전용 child를 RW로 mount한다. web만 host loopback publish를 위한 non-internal `loopback-edge`에 연결하고, service 통신은 `web-backend`, `build-internal`, `data-internal` 세 internal network로 분리한다. web은 UID/GID `101:101`과 같은 owner의 bounded tmpfs로 non-root 실행하며 required loopback port만 publish하고 `/api/admin/**`만 backend로 전달한다. project Nginx는 external HTTPS termination 뒤 생성하는 redirect를 relative `Location`으로 제한하고 backend에는 고정 `X-Forwarded-Proto: https`·`X-Forwarded-Port: 443`을 전달해 내부 HTTP scheme·port를 origin authority로 사용하지 않는다. normal backend/publisher의 Flyway와 bootstrap은 비활성이고, migration task만 Flyway를 활성한 후 Hibernate schema validate를 수행하며 schema·initial-admin·initial-content task는 Flyway 없이 validate를 수행한다. PostgreSQL은 project-scoped `postgres-data` named volume을 `/var/lib/postgresql`에 사용하고 host bind를 사용하지 않는다.

`compose.production.validation.yaml`은 base host source를 바꾸지 않고 validation run에서만 marker-owned temp public/media/state/build-workspace source를 덮어쓰고 one-shot task에 task label을 부여한다. `scripts/validate-production-compose.sh`는 rendered contract, actual migration·schema task, initial-admin의 exact argv·mount/port/credential 0과 non-TTY mutation 0, initial-content의 exact argv·bundle RO/media RW/DB-only network와 missing-admin mutation 0, malformed mode 거부, writer quiescence 중 static serving, actual mount mode·network·port, publisher workspace RW와 나머지 image source RO, static/admin/deny route, internal Build API authentication, 일반 `down` 뒤 sentinel·volume identity를 native architecture에서 검증한다. task container/network는 정리하고 task PostgreSQL volume은 삭제하지 않는다. 이 증거는 actual `/private/var/lib/rhaomi` ownership·permission이나 production volume·Secret·FQDN provisioning 및 실제 관리자·content 생성 증거가 아니다.
...
| `initial-admin` | Flyway 없이 JPA validate 후 TTY credential·transaction lock·zero-admin 단일 생성, one-shot | 금지 |
| `initial-content` | Flyway 없이 JPA validate 후 tracked bundle·pristine state·application validation/media normalization·single pending event, one-shot | 금지 |
...
| publisher build workspace | `/private/var/lib/rhaomi/state/publisher/build-workspace` | publisher `/opt/rhaomi/source/.rhaomi-publication-work` | host bind RW; 나머지 image source RO |
