...
local private media volume·upload API, same-origin development gateway, dedicated publisher control loop와 Build API→transformer→Next→immutable release/atomic switch data plane을 구현했다. network-disabled Node suites, explicit `publisher-validation`, `publication-acceptance` Java 25/Node 24 harness가 Sharp·filesystem symlink·DB completion에 더해 actual Admin HTTP·scheduled boundary·backend 중단 후 static serving을 amd64/arm64에서 검증한다. 위 production topology·lossless wire·verified-empty first activation 계약은 [ADR-010](../09-decisions/ADR-010-production-topology-and-code-release.md)~[ADR-016](../09-decisions/ADR-016-verified-empty-first-production-activation.md)에서 승인했다. D-IMP-1 decoder-only application image, D-IMP-2 `compose.production.yaml`·project Nginx, D-IMP-3 release/deploy, D-IMP-4 fixed backup/manifest/eligibility와 task-scoped isolated restore에 이어 no-port first-activation recovery Compose source를 구현했다. actual Secret·Mac ownership/path·production volume·backup repository/schedule·private GHCR·Environment·Tailscale·ingress/deploy·backup/restore·HomeOps provisioning은 여전히 미완료다.
