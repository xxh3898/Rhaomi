...
현재 local backend·gateway와 `/admin/` 인증 셸·미디어·매장정보·견종·서비스 관리 UI는 운영 배포 대상이 아니므로 2FA·TLS·운영 account provisioning을 구현하지 않는다. noindex와 client session 확인도 보안 통제로 간주하지 않으며, 실제 iPhone Safari HEIC upload·shop/견종/서비스 form·VoiceOver 증거가 없는 상태를 운영 준비 완료로 표현하지 않는다.
