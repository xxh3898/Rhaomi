...
| exact SHA tag 덮어쓰기·mutable apply | 같은 승인 식별자가 다른 byte를 가리킴 | required exact-head build arg, existing tag publish 거부, multi-arch manifest returned digest apply, requested digest·RepoDigest·OCI revision·runtime image ID 검증, `latest` 금지 |
| pre-publish SBOM을 published artifact 증거로 오인 | 검증한 byte와 배포한 manifest가 다르거나 platform attestation 누락 | published OCI index의 amd64/arm64 manifest·attestation descriptor, attached SPDX SBOM·SLSA provenance·OCI identity와 attached-SBOM scan을 machine-check하고 pre-publish evidence를 auxiliary로 분리 |
