...
2. 첫 accepted generation의 `claimedAt`부터 `T0 + 30s`를 포함하는 fixed debounce
...
- publisher state host source는 `/private/var/lib/rhaomi/state/publisher`, lock source는 `/private/var/lib/rhaomi/state/locks`다. 둘을 각각 publisher container `/var/lib/rhaomi/publisher`, `/var/lib/rhaomi/locks`로 read-write mount한다. image source는 read-only로 유지하고 `/private/var/lib/rhaomi/state/publisher/build-workspace`만 container `/opt/rhaomi/source/.rhaomi-publication-work`에 read-write mount해 Next/Turbopack workspace를 격리한다.
- D-IMP-2 `compose.production.yaml`은 backend와 동일 external image, exact non-web mode, internal Build API/DB network, public/state/lock/build-workspace RW와 media RO target을 구현했다. task overlay의 native smoke는 이 lifetime/mount 경계를 검증하지만 actual host source·Secret·approved digest provisioning은 아니다.
- D-IMP-3 fixed code deploy source는 global deploy lock 안에서 backend과 publisher를 정지·physical exit 확인한 뒤에만 one-shot Flyway/schema validation을 실행한다. writer maintenance 시작 뒤 migration/schema/backend health/publisher start/runtime image identity가 실패하면 두 writer를 다시 정지하고 physical quiescence 확인 뒤에만 own lock을 해제하며, 확인하지 못하면 lock을 보존한다. publisher를 자동 resume하지 않고 backend health 후에만 승인된 같은 digest의 publisher를 시작한다. 이는 actual GHCR·Environment·Tailscale·Mac provisioning·deploy 완료를 뜻하지 않는다.
- D-IMP-4 backup은 code deploy와 같은 host `rhaomi-deploy.lock`을 사용하고 publisher를 포함한 두 writer의 physical exit 뒤에만 DB/media snapshot을 시작한다. strict complete set 승격 뒤 같은 source image의 backend health·publisher running이 확인되어야 lock을 해제하며 복구 실패 시 lock을 보존한다. publisher executor의 별도 `publisher.lock` lifetime 계약은 그대로 유지한다.
