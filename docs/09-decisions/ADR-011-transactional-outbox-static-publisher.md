...
공개 build는 draft, 만료 공지, 보관된 relation과 private file을 누출하지 않아야 하고 실패한 build가 현재 정상 사이트를 덮어쓰지 않아야 한다. notice는 미래 `publishedAt`과 `expiresAt`을 허용하므로 저장 transaction 외에도 시간 경계 자체가 durable public build trigger여야 한다. 이 결정은 [ADR-003](ADR-003-static-publish-on-content-change.md)의 재빌드 방향을 구체화하며 이를 대체하지 않는다.
...
- notice create/update/publish transaction이 미래 `publishedAt` 또는 미래 `expiresAt`을 만들면 같은 transaction에 해당 boundary의 durable scheduled publishing event를 기록한다.
- scheduled event에는 최소 event kind, `availableAt` 또는 동등한 `notBefore`, notice ID, event를 만든 current notice/content revision과 boundary 값을 기록한다.
...
- response는 일관된 `contentRevision`, target `publishGeneration`과 `generatedAt`을 포함한다.
...
- scheduled event 처리 직전 current notice row와 전체 build snapshot을 다시 읽어 status, current `publishedAt`·`expiresAt`, relation, media/file과 `generatedAt` eligibility를 재검증한다.
...
- transactional outbox는 콘텐츠 commit과 publish 요청 기록의 유실 구간을 없애고, durable scheduled event는 mutation이 없는 게시·만료 시각의 trigger 유실을 막는다.
...
- [ ] immediate·scheduled outbox, `availableAt`, claim 복구와 두 revision의 Flyway·transaction 분류 구현
