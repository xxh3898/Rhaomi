...
| FR-DEP-010 | generation을 가진 active claim의 완료·lease 갱신은 current owner와 generation을 확인해야 하며, lower generation은 존재하는 higher active generation으로만 coalesce할 수 있어야 한다. dedicated publisher는 첫 accepted generation 기준 고정 30초 debounce와 global filesystem lock 안에서 highest generation만 실행해야 한다. |
...
| FR-DEP-014 | Static Export는 generated V2 snapshot을 runtime API 호출 없이 홈·정적 공지 상세·canonical·sitemap·robots·responsive media에 바인딩하고, Notice Markdown의 raw HTML·dangerous URL·remote image 우회를 허용하지 않아야 한다. |
| FR-DEP-015 | release는 content revision·publish generation·generatedAt·code identity·site digest를 strict manifest에 기록하고, current generation을 `BigInt`로 다시 비교한 뒤에만 `previous/current`를 same-filesystem atomic rename으로 전환해야 한다. |
| FR-DEP-016 | candidate·post-switch serving smoke 실패는 기존 current를 유지하거나 복구하고 publication success를 기록하지 않아야 한다. current·previous와 최근 성공 release 5개는 retention에서 보호해야 한다. |
| FR-DEP-017 | Java publisher executor는 credential을 environment에만 두고 fixed argv Node CLI의 safe single-line result만 수용해야 한다. cancellation 시 child·descendant physical exit와 Java body 종료가 확인되기 전에 global lock을 해제하지 않아야 한다. |
