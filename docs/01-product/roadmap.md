...
- ADR-015 Java/PostgreSQL int64의 canonical decimal-string wire·generated artifact 계약
- ADR-016 verified-empty one-time bootstrap·first backup/isolated restore 뒤 steady-state 전환
- ADR-017 exact-image TTY-only production initial-admin·PostgreSQL zero-admin single-use authority
- [Production readiness matrix](../07-operations/production-readiness.md)의 계약·local/CI·구현·provisioning·외부 승인·물리 acceptance 분리

Phase 1D는 `Production operating architecture CONTRACT COMPLETE`다. D-IMP-1 canonical decoder-only image, D-IMP-2 external-image production Compose/project Nginx, D-IMP-3 release/deploy gate, D-IMP-4 shared-lock application-consistent backup·manifest/eligibility·isolated restore, D-IMP-5a Rhaomi bounded adapter/target와 D-IMP-5b HomeOps incident decision·V14 mapping/audit·durable cooldown, D-IMP-6의 verified-empty first-activation source gate를 구현했다. 이는 `Production provisioning/deploy COMPLETE`가 아니다. 실제 Mac directory ownership·permission, production bind·volume·Secret·FQDN, backup repository·schedule·production first activation/backup/restore/RPO·RTO, private GHCR package, protected GitHub Environment, Tailscale identity, Rhaomi web mapping·Agent capability와 approved image digest 배치는 후속 production gate에서 provisioning·검증한다.
