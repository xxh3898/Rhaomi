...
| initial local-only backup / restore | [ADR-012](../09-decisions/ADR-012-application-consistent-backup-restore.md), [백업·복구](backup-and-restore.md) | fixed shared-lock backup source, strict manifest·atomic complete set, release eligibility bridge, retention guard와 A→B→isolated A restore·static/restart/down-up task evidence | `PROVISIONING_REQUIRED` | 예 | protected source와 분리된 actual Mac repository/path·owner·capacity, 03:30 scheduler installation, production DB/media backup과 first-production restore/RPO·RTO evidence |
