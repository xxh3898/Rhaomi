...
| code release / image / SBOM | [ADR-010](../09-decisions/ADR-010-production-topology-and-code-release.md) | canonical image, workflow_dispatch-only exact-main multi-arch/SBOM/provenance source, protected Environment job, pinned Tailscale/fixed SSH argv, Mac deploy lock·digest/revision·writer quiescence·failure hold task evidence | `PROVISIONING_REQUIRED` | 예 | private GHCR package/visibility, actual Environment reviewer/main policy/secret, Tailscale identity·host install, approved backup gate, exact digest Mac apply |
...
D-IMP-1 canonical image, D-IMP-2 production Compose/project Nginx, D-IMP-3 exact-main release workflow·fixed deploy entrypoint·one-shot Flyway/schema task source와 task-scoped local/Hosted validator는 구현·검증됐다. actual `/private/var/lib/rhaomi`, production named volume, private GHCR package/visibility, GitHub `production` Environment·reviewer·secret, Tailscale identity, Mac entrypoint/config, Secret·FQDN·host edge·Cloudflare는 `PROVISIONING_REQUIRED`다. production workflow dispatch·package push·deploy·migration은 수행하지 않았다.
...
D-IMP-3는 release/deploy workflow·entrypoint·one-shot task source와 synthetic/task-scoped validation만 구현한다. 실제 GitHub Environment·GHCR package·Tailscale identity, Mac canonical path·entrypoint·production volume·Secret·FQDN·HomeOps·Cloudflare를 생성·변경하지 않았고 workflow dispatch, package push, merge, release, deploy, production migration·mutation도 수행하지 않았다.
