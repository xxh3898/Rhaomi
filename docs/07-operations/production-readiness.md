...
| production 초기 콘텐츠 authority | [ADR-018](../09-decisions/ADR-018-production-initial-content-authority.md), [배포](deployment.md) | tracked exact bundle schema, fixed non-web task, application validation/media normalization, pristine lock·single transaction·one pending event, replay/concurrency/rollback의 source/task evidence | `LOCAL_CI_VERIFIED` | 예 | #97 integration·Source Release 후 actual owner bundle 승인/transport, pristine host import와 canonical publisher first release acceptance |
...
| verified-empty first activation | [ADR-016](../09-decisions/ADR-016-verified-empty-first-production-activation.md), [배포](deployment.md) | one-time verified-empty→private bootstrap→first backup→no-port isolated restore→`STEADY_STATE` lifecycle source와 task contract | `PROVISIONING_REQUIRED` | 예 | actual Mac inventory의 absence evidence, exact image private bootstrap, first backup/full-read/isolated restore acceptance; public/admin/content activation은 이후 별도 gate |
| initial local-only backup / restore | [ADR-012](../09-decisions/ADR-012-application-consistent-backup-restore.md), [백업·복구](backup-and-restore.md) | fixed shared-lock backup source, strict manifest·atomic complete set, `<24h` release eligibility·manifest freshness, first-activation candidate와 mount-level read-only verifier, retention guard와 A→B→isolated A restore·static/restart/down-up task evidence | `PROVISIONING_REQUIRED` | 예 | protected source와 분리된 actual Mac repository/path·owner·capacity, 03:30 scheduler installation, production DB/media first backup·isolated restore와 RPO·RTO evidence |
...
| HomeOps monitoring / recovery | [ADR-013](../09-decisions/ADR-013-homeops-monitoring-recovery-boundary.md), [모니터링](monitoring-and-incident-response.md) | HomeOps `main@0a8ce9090c76f5ad7afba19ca896e923b96b0cbf`, run `33569523762` application deploy·V14 `APPLIED`, Agent artifact `PUBLISHED`와 Agent rollout `NOT_RUN`; current status-only/web-only activation preflight | `PROVISIONING_REQUIRED` | 예 | public HTTPS expected status 3회→`rhaomi-web` disabled mapping·Rhaomi fixed inventory·Agent rollout/fresh capability provisioning; backend mapping 없음, enable/restart/drill은 별도 승인 |
...
- initial-admin/initial-content source와 CI 성공은 실제 administrator·owner content·first public release 증거가 아니다. 두 Issue의 dev integration과 Source Release/back-sync/exact-main 재검증 전에 Issue #95 Stage A로 진행하지 않는다.
...
Admin second factor의 `PROVISIONING_REQUIRED`는 source 구현 완료만 뜻한다. 실제 FQDN/RP/origin을 커밋하지 않았고 운영 관리자 account·passkey·recovery code도 생성하지 않았으므로 `overallProductionReadiness=HOLD`와 password-only production 금지를 유지한다.
...
| 6 | D-IMP-6 | verified-empty first activation + first backup/isolated restore + actual domain/content/public HTTPS/iPhone Safari·VoiceOver/rollback·recovery | predecessor 없는 최초 host를 private recovery acceptance까지 올린 뒤 실제 외부값·물리 기기·운영 복구를 최종 확인한다. |
...
D-IMP-1 canonical image, D-IMP-2 production Compose/project Nginx, D-IMP-3 exact-main release workflow·fixed deploy entrypoint·one-shot Flyway/schema task, D-IMP-4 shared-lock backup/manifest/eligibility/isolated restore, D-IMP-5a Rhaomi adapter/target와 D-IMP-5b HomeOps incident decision·V14 mapping/audit·durable cooldown source는 구현·검증됐다. D-IMP-5b production 순서는 `HomeOps release → live compatibility 재검증 → Rhaomi release/provisioning`이다. 앞의 두 단계는 HomeOps `main@0a8ce9090c76f5ad7afba19ca896e923b96b0cbf`와 run `33569523762` 근거로 완료했고, Rhaomi release/provisioning은 `NOT_RUN`이다. HomeOps application과 V14는 배포됐고 Agent artifact는 `PUBLISHED`지만 Agent rollout은 `NOT_RUN`이다. Actual Rhaomi fixed inventory, monitor/mapping/control/notification, Agent capability, production named volume·backup repository·schedule, private GHCR package/visibility, GitHub `production` Environment·reviewer·secret, Tailscale identity, Mac entrypoint/config, Secret·FQDN·host edge·Cloudflare는 미완료다.
...
HomeOps application release/deploy와 V14는 별도 HomeOps workflow에서 완료됐고 Rhaomi는 이를 read-only로 재검증했다. 이 Rhaomi 작업은 실제 GitHub Environment·GHCR package·Tailscale identity, Mac canonical path·entrypoint·production volume·backup repository·schedule·Secret·FQDN·HomeOps mapping·Cloudflare를 생성·변경하지 않았다. Rhaomi workflow dispatch, package push, merge, release, deploy, production migration·backup·restore, mapping create/enable, Agent rollout, restart/drill·mutation도 수행하지 않았다. Overall production readiness는 계속 `HOLD`다.
